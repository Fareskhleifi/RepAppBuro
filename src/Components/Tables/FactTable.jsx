...
export default function FactTable() {
  ...
  const fetchFactures = async () => {
    try {
      ...
      const response = await request("get", "/rsc/getFacturesActives", null, headers);
  ...
  const handleArchiver = async (invoice) =>{
    try {
      const token = sessionStorage.getItem('token') || Cookies.get('token');
      const headers = { Authorization: `Bearer ${token}` };
      await request("put", `/rsc/archiverFacture?id=${invoice.idFacture}`, null,headers);
      toast.success("facture archivée avec succès!", { autoClose: 1300 });
      fetchFactures();
    } catch (error) {
      toast.error("Erreur lors de l'archivage de la facture.");
  ...
  const handleClick = (invoice)=>{
    MySwal.fire({
      title: 'Confirmer la archivage',
      text: "Êtes-vous sûr de vouloir d'archiver la facture ?",
      icon: 'question',
      showCancelButton: true,
                  ...
                  {filteredInvoices.length > 0 ? (
                    filteredInvoices.slice((currentPage - 1) * 5, currentPage * 5).map(invoice => (
                      <tr key={invoice.idFacture}>
                        ...
                        <td className="px-4 py-4 text-sm text-gray-500 dark:text-gray-300">{(invoice.montantTotal).toFixed(2) } (TND)</td>
                        <td className="px-4 py-4 text-sm whitespace-nowrap">
                          <div className="flex items-center gap-x-6">
                            {
                              !invoice.archived &&(
                            <button onClick={()=>handleClick(invoice)} className="text-gray-500 hover:text-indigo-500 focus:outline-none">Archive</button>
                              )}
